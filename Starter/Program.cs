...
do
{
    ...
    switch (menuSelection)
    {
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 2] == "Age: ?" && ourAnimals[i, 0] != "ID #: ")
                {
                    do
                    {
                        ...
                        {
                            animalAge = readResult;
                            validEntry = int.TryParse(animalAge, out petAge); // converts and validates that the age input is an integer
                        }
                    } while (validEntry == false);
                    ourAnimals[i, 2] = "Age: " + animalAge;
                }
        ...
        case "4":
            ...
            Console.WriteLine("\n\rnickname and pesonality fields are complete for all of our friends. \n\rPress the Enter key to continue");
            readResult = Console.ReadLine();
